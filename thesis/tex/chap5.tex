...
Upon partitioning the founder graph, we find that there are 584 communities, with an average of 1.2 founders per community. Our model of the founder community on the platform was not accurate.
...
In order to have a model to compare against, we first start with a random model, which simply generates a score randomly and uniformly in $[0, 1]$ for each founder, then ranks them based on this score. While the random model would never be used for a serious application, it is worth noting that this way of sorting founders is roughly what is used by analysts in the real world today, who haphazardly pick new companies to investigate based on ``gut feelings''. The metrics for our random model are found in Figure \ref{fig:rand:results}.
...
The takeaway of access being a driving factor to founder fundraising success further motivates our work on the VCWiz platform. A stated goal of the platform is to increase founders' access to investors, a demonstrably high-impact outcome.
...
The rationale for these weights follows from the metrics from \cite{2017arXiv170604229H} which have been shown to correlate with a founder's success rate, including ability to successfully fundraise. We additionally add the Aggregate Funding feature from the last experiment, to acknowledge tat founder who have raised a large amount of capital in the past are more likely to be trusted as stewards of capital in the future.
...
\textbf{TODO: best model with investment data results}

\subsection{Broader Venture Ecosystem}

\textbf{TODO: best model with investment data and cofound/coinvest data results}
...
% - Look at correlation between things. Why is this result much less good? Graph is not representative? Something else going on? Have to sit and think, and also do it on graph from 4. Graph misses nos, which are crucial part of the ecosystem. That's just how the came goes.

\subsection{FR+I with Email Data}

\textbf{TODO: add back in vcwiz email nodes and edges}
